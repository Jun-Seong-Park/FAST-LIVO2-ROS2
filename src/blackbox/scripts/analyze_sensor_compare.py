...
STREAM_MAP = {
    "lidar": {
        ...
        "pub_dtype": PUB_24,
        "pub_t_col": "t_pub_ns",
        "fps":      10,    # publisher rate (Hz) — for period-based thresholds
    },
    "imu": {
...
# Status thresholds (period-relative for latency, absolute for drop rate).
# Same model across streams → stream-specific fine-tuning not needed.
LAT_GOOD_PCT  = 10.0    # P99 < 10% of period
LAT_WARN_PCT  = 30.0    # P99 < 30% of period
DROP_GOOD_PCT = 0.0     # peak drop rate (1s window)
DROP_WARN_PCT = 5.0     # peak < 5%
STATUS_COLOR  = {'GOOD': 'C2', 'WARN': 'C1', 'FAIL': 'C3'}


def latency_status(p99_ms: float, period_ms: float) -> str:
    if period_ms <= 0:
        return 'GOOD'
    pct = 100.0 * p99_ms / period_ms
    if pct < LAT_GOOD_PCT:
        return 'GOOD'
    if pct < LAT_WARN_PCT:
        return 'WARN'
...
def drop_status(peak_pct: float) -> str:
    if peak_pct <= DROP_GOOD_PCT:
        return 'GOOD'
    if peak_pct < DROP_WARN_PCT:
        return 'WARN'
    return 'FAIL'
...
def cmp_plot(stream: str, pub: np.ndarray, sub: np.ndarray,
    ...
    # ── status thresholds ─────────────────────────────────────────────────────
    fps        = cfg.get('fps', 0)
    period_ms  = 1000.0 / fps if fps > 0 else 0.0
    p99_ms_val = float(np.percentile(lat_ms, 99))
    lat_stat   = latency_status(p99_ms_val, period_ms)
    peak_drop  = float(dr_rate.max()) if len(dr_rate) else 0.0
    drop_stat  = drop_status(peak_drop)

    ax1.set_title(
        f'{stream.upper()}  Latency  (t_cbk_sub - t_pub)  '
        f'[{lat_stat}]',
        fontsize=14, fontweight='bold', loc='center', pad=12,
        color=STATUS_COLOR[lat_stat])
    ...
    ax2.set_title(
        f'{stream.upper()}  Drop Rate  (window = {win_s:g} s)  '
        f'[{drop_stat}]',
        fontsize=14, fontweight='bold', loc='center', pad=12,
        color=STATUS_COLOR[drop_stat])
    ...
    period_disp = f'{period_ms:.2f} ms (1/{fps} Hz)' if fps > 0 else 'N/A'
    fig.text(0.5, -0.02,
             f'Status thresholds — period = {period_disp}.  '
             f'Latency: GOOD P99<{LAT_GOOD_PCT:g}% of period, '
             f'WARN <{LAT_WARN_PCT:g}%, FAIL ≥{LAT_WARN_PCT:g}%.  '
             f'Drop rate (peak in {win_s:g}s window): GOOD 0%, '
             f'WARN <{DROP_WARN_PCT:g}%, FAIL ≥{DROP_WARN_PCT:g}%.',
             ha='center', fontsize=9, color='gray')

    _savefig(fig, out_path)
    print(f'  saved {out_path}  [lat={lat_stat} drop={drop_stat}]')
