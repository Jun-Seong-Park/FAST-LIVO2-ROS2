...
/**
 * @file blackbox.hpp
 * @brief Blackbox (econ_ros2_driver vendored subset) — image-publish stream only.
 *
...
struct ImagePubRecord {
  ...
  uint64_t t_cbk_ns;         // 8B  log_cbk timestamp
  uint64_t t_pub_ns;         // 8B  log_pub timestamp
  uint8_t  cap_done;         // 1B  flag
  uint8_t  ros_done;         // 1B  flag
  uint8_t  _pad[6];          // 6B
  // spans:  capture→cbk = t_cbk_ns - t_capture_ns,  cbk→pub = t_pub_ns - t_cbk_ns
};
