...
namespace {
uint64_t StampNs(const builtin_interfaces::msg::Time &stamp) {
...
void LogSubGap(
    ...
    double prev_stamp,
    double warn_gap_s) {
  if (count > 1 && prev_stamp > 0.0) {
    const double dt = stamp - prev_stamp;
    if (dt < -0.001 || dt > warn_gap_s) {
      RCLCPP_WARN(
          logger,
          "[SUB-GAP] %s cb=%lu stamp=%.9f prev=%.9f dt=%.6f threshold=%.6f",
          name,
...
LIVMapper::~LIVMapper() {
  try { savePCD(); } catch (...) {}
  blackbox::sub_lidar::shutdown();
  blackbox::sub_imu::shutdown();
  blackbox::sub_image::shutdown();
}
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  this->node->declare_parameter<std::string>("common.img_topic", "/arducam/image");
  ...
  // camera parameters (loaded from camera_arducam.yaml, used by camera_loader)
  ...
  this->node->declare_parameter<double>("cam_d4", 0.0);
...
void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr &node) 
{
...

  {
    ...
    if (cam_model == "Pinhole")
    {
      ...
      double scale = 1.0, cam_fx = 0, cam_fy = 0, cam_cx = 0, cam_cy = 0;
      double cam_d0 = 0, cam_d1 = 0, cam_d2 = 0, cam_d3 = 0, cam_d4 = 0;
      this->node->get_parameter("cam_width",  cam_width);
      this->node->get_parameter("cam_height", cam_height);
      this->node->get_parameter("scale",      scale);
      ...
      this->node->get_parameter("cam_d0",     cam_d0);
      this->node->get_parameter("cam_d1",     cam_d1);
      this->node->get_parameter("cam_d2",     cam_d2);
      this->node->get_parameter("cam_d3",     cam_d3);
      this->node->get_parameter("cam_d4",     cam_d4);
      vio_manager->cam = new vk::PinholeCamera(
    ...
    {
      throw std::runtime_error("Camera model not correctly specified: " + cam_model);
    }
  }
...
{
  {
    ...
    pubImage = it.advertise("/rgb_img", rmw_qos_profile_default);
  }

  auto sensor_qos = rclcpp::QoS(200000).best_effort().durability_volatile();
  RCLCPP_INFO(
      this->node->get_logger(),
      "[SUB-SETUP] lidar_topic=%s imu_topic=%s image_topic=%s qos=best_effort depth=200000",
      lid_topic.c_str(),
      imu_topic.c_str(),
      img_topic.c_str());
...
{
  blackbox::sub_lidar::log(StampNs(msg->header.stamp));
  static uint64_t cb_count = 0;
  static double prev_stamp = -1.0;
  cb_count++;
  const double raw_stamp = stamp2Sec(msg->header.stamp);
  LogSubGap(this->node->get_logger(), "lidar_pc2", cb_count, raw_stamp, prev_stamp, 0.25);
  prev_stamp = raw_stamp;
  RCLCPP_INFO_THROTTLE(
      this->node->get_logger(), *this->node->get_clock(), 1000,
      "[SUB-RX] lidar_pc2 cb=%lu stamp=%.9f last_lidar=%.9f q_lidar=%zu q_imu=%zu q_img=%zu points_width=%u",
  ...
  // Keep only latest frames to prevent buffer overflow
  const size_t MAX_LIDAR_BUFFER = 3;
  while (lid_raw_data_buffer.size() > MAX_LIDAR_BUFFER) {
    lid_raw_data_buffer.pop_front();
    lid_header_time_buffer.pop_front();
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 1000,
      "Dropping LiDAR frame - processing too slow (buffer: %zu)", lid_raw_data_buffer.size());
  }
...
{
  blackbox::sub_lidar::log(StampNs(msg_in->header.stamp));
  static uint64_t cb_count = 0;
  static double prev_stamp = -1.0;
  cb_count++;
  const double raw_stamp = stamp2Sec(msg_in->header.stamp);
  LogSubGap(this->node->get_logger(), "lidar_custom", cb_count, raw_stamp, prev_stamp, 0.25);
  prev_stamp = raw_stamp;
  RCLCPP_INFO_THROTTLE(
      this->node->get_logger(), *this->node->get_clock(), 1000,
      "[SUB-RX] lidar_custom cb=%lu stamp=%.9f timebase=%lu last_lidar=%.9f q_lidar=%zu q_imu=%zu q_img=%zu point_num=%u",
      static_cast<unsigned long>(cb_count),
      raw_stamp,
      static_cast<unsigned long>(msg_in->timebase),
      last_timestamp_lidar,
      lid_raw_data_buffer.size(),
      imu_buffer.size(),
      img_buffer.size(),
...
{
  blackbox::sub_imu::log(StampNs(msg_in->header.stamp));
  static uint64_t cb_count = 0;
  static double prev_stamp = -1.0;
  cb_count++;
  const double raw_stamp = stamp2Sec(msg_in->header.stamp);
  LogSubGap(this->node->get_logger(), "imu", cb_count, raw_stamp, prev_stamp, 0.05);
  prev_stamp = raw_stamp;
  RCLCPP_INFO_THROTTLE(
      this->node->get_logger(), *this->node->get_clock(), 1000,
      "[SUB-RX] imu cb=%lu stamp=%.9f last_lidar=%.9f last_imu=%.9f lidar_minus_imu=%.6f q_lidar=%zu q_imu=%zu q_img=%zu",
      static_cast<unsigned long>(cb_count),
      raw_stamp,
      last_timestamp_lidar,
      last_timestamp_imu,
      last_timestamp_lidar - raw_stamp,
      lid_raw_data_buffer.size(),
      imu_buffer.size(),
      img_buffer.size());
...
{
  cv::Mat img;
  if (img_msg->encoding == "nv12" || img_msg->encoding == "NV12")
  {
    // NV12: Y plane (height rows) + interleaved UV plane (height/2 rows)
    // Total data size = width * height * 3/2
  ...
  RCLCPP_DEBUG_THROTTLE(
      this->node->get_logger(), *this->node->get_clock(), 1000,
      "[SUB-CV] img.data=%p msg.data=%p same=%d owns=%d enc=%s",
      static_cast<void*>(img.data),
      static_cast<const void*>(img_msg->data.data()),
      img.data == reinterpret_cast<const uchar*>(img_msg->data.data()),
      img.u != nullptr,
                        ...
                        const rclcpp::MessageInfo &info)
{
  ...
  cb_count++;
  const double raw_stamp = stamp2Sec(msg_in->header.stamp);
  LogSubGap(this->node->get_logger(), "image", cb_count, raw_stamp, prev_stamp, 0.25);
  prev_stamp = raw_stamp;
  RCLCPP_INFO_THROTTLE(
      this->node->get_logger(), *this->node->get_clock(), 1000,
      "[SUB-RX] image cb=%lu stamp=%.9f last_img=%.9f last_lidar=%.9f image_minus_lidar=%.6f q_lidar=%zu q_imu=%zu q_img=%zu size=%ux%u enc=%s",
      static_cast<unsigned long>(cb_count),
      raw_stamp,
      last_timestamp_img,
      last_timestamp_lidar,
      raw_stamp - last_timestamp_lidar,
      lid_raw_data_buffer.size(),
      imu_buffer.size(),
      img_buffer.size(),
      msg_in->width,
      msg_in->height,
      msg_in->encoding.c_str());
  if (!img_en) return;
  // RCLCPP_INFO(this->node->get_logger(), "get image at time: %.6f", stamp2Sec(msg_in->header.stamp));
  ...
  if (abs(msg_header_time - last_timestamp_img) < 0.001) {
    RCLCPP_WARN_THROTTLE(
        this->node->get_logger(), *this->node->get_clock(), 1000,
        "[SUB-DROP] image duplicate_or_too_close cb=%lu stamp=%.9f last_img=%.9f diff=%.9f",
        static_cast<unsigned long>(cb_count),
        msg_header_time,
        last_timestamp_img,
        msg_header_time - last_timestamp_img);
    return;
  }
  ...
  // Keep only the latest frame - drop old ones immediately
  while (img_buffer.size() > 1) {
