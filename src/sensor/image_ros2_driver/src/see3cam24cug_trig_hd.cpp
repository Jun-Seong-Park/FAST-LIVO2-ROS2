// See3CAM_24CUG — HD (1280x720) variant. GStreamer NVMM → BGRx → bgr8.
// blackbox publish trace: blackbox::image (mmap-fixed-array + 1s writer thread).
// Pipeline:
//   v4l2src /dev/24cug ! UYVY 1280x720@60 ! nvvidconv ! BGRx 1280x720 ! videoconvert ! BGR ! appsink
...
class See3cam24cugTrigHd : public rclcpp::Node
{
 public:
      ...
      diag_sleep_ms_(read_env_int("SEE3CAM_DIAG_SLEEP_MS", cfg::kDiagSleepMs))
  {
    ...
    publish_thread_ = std::thread(&See3cam24cugTrigHd::publish_loop, this);

    set_trigger_srv_ = create_service<std_srvs::srv::SetBool>(
      "~/set_trigger_mode",
      std::bind(&See3cam24cugTrigHd::on_set_trigger, this,
                std::placeholders::_1, std::placeholders::_2));
    get_trigger_srv_ = create_service<std_srvs::srv::Trigger>(
      "~/get_trigger_mode",
      std::bind(&See3cam24cugTrigHd::on_get_trigger, this,
                std::placeholders::_1, std::placeholders::_2));
 ...
 private:
  ...
  static GstFlowReturn on_new_sample_static(GstElement *sink, gpointer data) {
    return static_cast<See3cam24cugTrigHd *>(data)->on_new_sample(sink);
  }
  ...
  GstFlowReturn on_new_sample(GstElement *sink) {
    ...
    const uint64_t t_push = t_dqbuf_ns_;
  ...
  static gboolean on_bus_message_static(GstBus *, GstMessage *msg, gpointer data) {
    return static_cast<See3cam24cugTrigHd *>(data)->on_bus_message(msg);
  }
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<See3cam24cugTrigHd>());
  rclcpp::shutdown();
  return 0;
}
