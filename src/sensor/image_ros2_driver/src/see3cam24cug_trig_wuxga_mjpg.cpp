// See3CAM_24CUG — WUXGA MJPG (1920x1200) variant. GStreamer NVMM → BGRx → bgr8.
// blackbox publish trace: blackbox::image (mmap-fixed-array + 1s writer thread).
// Pipeline:
//   v4l2src /dev/24cug ! MJPG 1920x1200@60 ! nvvidconv ! BGR 1920x1200 ! videoconvert ! BGR ! appsink
...
constexpr const char* kNodeName       = "see3cam24cug_trig_wuxga_mjpg";
constexpr const char* kLogLabel       = "[see3cam24cug_wuxga_mjpg]";
const std::string kBlackboxPath = blackbox::trace_log_dir() + "/see3cam24cug_wuxga_mjpg_image_pub.bin";
}  // namespace

class See3cam24cugTrigWuxgaMjpg : public rclcpp::Node
{
 public:
  See3cam24cugTrigWuxgaMjpg()
    : Node(kNodeName),
      ...
      diag_sleep_ms_(read_env_int("SEE3CAM_DIAG_SLEEP_MS", cfg::kDiagSleepMs))
  {
    ...
    publish_thread_ = std::thread(&See3cam24cugTrigWuxgaMjpg::publish_loop, this);

    set_trigger_srv_ = create_service<std_srvs::srv::SetBool>(
      "~/set_trigger_mode",
      std::bind(&See3cam24cugTrigWuxgaMjpg::on_set_trigger, this,
                std::placeholders::_1, std::placeholders::_2));
    get_trigger_srv_ = create_service<std_srvs::srv::Trigger>(
      "~/get_trigger_mode",
      std::bind(&See3cam24cugTrigWuxgaMjpg::on_get_trigger, this,
                std::placeholders::_1, std::placeholders::_2));
  ...
  ~See3cam24cugTrigWuxgaMjpg() override {
    shutdown();
 ...
 private:
  ...
  static GstFlowReturn on_new_sample_static(GstElement *sink, gpointer data) {
    return static_cast<See3cam24cugTrigWuxgaMjpg *>(data)->on_new_sample(sink);
  }
  ...
  static gboolean on_bus_message_static(GstBus *, GstMessage *msg, gpointer data) {
    return static_cast<See3cam24cugTrigWuxgaMjpg *>(data)->on_bus_message(msg);
  }
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<See3cam24cugTrigWuxgaMjpg>());
  rclcpp::shutdown();
  return 0;
}
